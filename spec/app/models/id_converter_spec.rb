...
describe IdConverter do
  ...
  xit 'should return movie repo when last 2 digits are 00' do
    id = 12_300
    repo = converter.get_repo(id)
    expect(repo).to be_instance_of(Persistence::Repositories::MovieRepo)
  end

  xit 'should return tv show repo when last 2 digits are 01' do
    id = 112_301
    repo = converter.get_repo(id)
    expect(repo).to be_instance_of(Persistence::Repositories::TvShowRepo)
  end

  xit 'should return episode repo when last 2 digits are 02' do
    id = 212_302
    repo = converter.get_repo(id)
